//  Another STL-like templated tree class.
//  Revised by: Xinlin Cao(xinlincao@gmail.com)
//  Revised from Kasper Peeters's implementation.(http://www.aei.mpg.de/~peekas/tree/),Kasper Peeters <kasper.peeters@aei.mpg.de>
//Compared to Kasper's implementation, the modifications are the following:
...
//# change some parameters of some functions, such as append_child, isnert_xxx, move_xxx and so on.
...
public:
	...
	~tree();
	tree<T, tree_node_allocator>& operator=(
	...
	/// Return iterator to the beginning of the tree.
	inline pre_order_iterator begin_pre_order_iterator(
			const iterator_base&) const;
	/// Return iterator to the end of the tree.
	inline pre_order_iterator end_pre_order_iterator(
			const iterator_base&) const;
	...
	breadth_first_queued_iterator begin_breadth_first_iterator(
			const iterator_base&) const;
	/// Return breadth-first end iterator.
	breadth_first_queued_iterator end_breadth_first_iterator(
			const iterator_base&) const;
	...
	pre_order_iterator move_after(const iterator_base& target,
			const iterator_base& source);
	/// Move 'source' node (plus its children) to become the previous sibling of 'target'.
	pre_order_iterator move_before(const iterator_base& target,
			const iterator_base& source);
...
private:
	...
	void construct_children_nodes_helper_(const pre_order_iterator& dst,
			const tree<T, tree_node_allocator>& other,
			const pre_order_iterator& src);
...
template<class T, class tree_node_allocator>
tree<T, tree_node_allocator>& tree<T, tree_node_allocator>::operator=(
		const tree<T, tree_node_allocator>& other) {
	if (this != &other) {
		copy_(other);
	}
...
template<class T, class tree_node_allocator>
void tree<T, tree_node_allocator>::construct_children_nodes_helper_(
		const pre_order_iterator& dst,
		const tree<T, tree_node_allocator>& other,
		const pre_order_iterator& src) {

	children_iterator childIter = other.begin_children_iterator(src);
	for (; childIter != other.end_children_iterator(other.root());
			++childIter) {
...
#define CXL_IMP 0
	...
	children_iterator childIter = this->begin_children_iterator(it);
	children_iterator childIterNext;
	while(childIter != this->end_children_iterator(it))
	{
		childIterNext = ++childIter;
		erase(childIter);
		childIter = childIterNext;
	}
...
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::root() const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::begin_pre_order_iterator(
		const iterator_base& pos) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::end_pre_order_iterator(
		const iterator_base& pos) const {
...
typename tree<T, tree_node_allocator>::breadth_first_queued_iterator tree<T,
		tree_node_allocator>::begin_breadth_first_iterator(
		const iterator_base& pos) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::breadth_first_queued_iterator tree<T,
		tree_node_allocator>::end_breadth_first_iterator(
		const iterator_base& pos) const {
...
typename tree<T, tree_node_allocator>::post_order_iterator tree<T,
		tree_node_allocator>::begin_post_order_iterator(
		const iterator_base& pos) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::post_order_iterator tree<T,
		tree_node_allocator>::end_post_order_iterator(
		const iterator_base& pos) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T,
		tree_node_allocator>::begin_children_iterator(
		const iterator_base& pos) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T,
		tree_node_allocator>::end_children_iterator(
		const iterator_base& pos) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator tree<T, tree_node_allocator>::begin_leaf_iterator(
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::leaf_iterator tree<T, tree_node_allocator>::end_leaf_iterator(
		const iterator_base& top) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::parent(const iterator_base& position) {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::previous_sibling(
		const iterator_base& position) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::next_sibling(
		const iterator_base& position) const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::append_child(const iterator_base& position,
		const T& x) {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::prepend_child(const iterator_base& position,
		const T& x) {
	assert(position.node != head);
	assert(position.node);
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::set_root(const T& x) {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::insert_before(iterator_base& position,
		const T& x) {
	if (position.node == 0) {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::insert_after(const iterator_base& position,
		const T& x) {
	tree_node* tmp = alloc_.allocate(1, 0);
	kp::constructor(&tmp->data, x);
	tmp->first_child = 0;
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::modify_data(const iterator_base& position,
		const T& x) {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::reparent_root(const T& x) {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::move_after(const iterator_base& target,
		const iterator_base& source) {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::move_before(const iterator_base& target,
		const iterator_base& source) {
	tree_node *dst = target.node;
	tree_node *src = source.node;
	assert(dst);
...
size_t tree<T, tree_node_allocator>::size() const {
	size_t i = 0;
	pre_order_iterator it = begin_pre_order_iterator(NULL), eit =
			end_pre_order_iterator(NULL);
	while (it != eit) {
		++i;
		++it;
	}
...
int tree<T, tree_node_allocator>::max_depth() const {
	int maxd = -1;
	for (tree_node *it = head->next_sibling; it != feet; it =
			it->next_sibling) {
...
template<class T, class tree_node_allocator>
bool tree<T, tree_node_allocator>::children_iterator::operator!=(
		const children_iterator& other) const {
	if (other.node != this->node)
		return true;
	else
		return false;
...
template<class T, class tree_node_allocator>
bool tree<T, tree_node_allocator>::leaf_iterator::operator!=(
		const leaf_iterator& other) const {
	if (other.node != this->node)
		return true;
	else
		return false;
...
bool tree<T, tree_node_allocator>::leaf_iterator::operator==(
		const leaf_iterator& other) const {
	if (other.node == this->node && other.top_node == this->top_node)
		return true;
	else
		return false;
}
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T,
		tree_node_allocator>::iterator_base::begin_children_iterator() const {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T,
		tree_node_allocator>::iterator_base::end_children_iterator() const {
	children_iterator ret(0);
	ret.parent_ = node;
	return ret;
}
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::pre_order_iterator tree<T,
		tree_node_allocator>::pre_order_iterator::operator++(int) {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::breadth_first_queued_iterator tree<T,
		tree_node_allocator>::breadth_first_queued_iterator::operator++(int) {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator& tree<T,
		tree_node_allocator>::children_iterator::operator++() {
	if (this->node)
		this->node = this->node->next_sibling;
	return *this;
}
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator tree<T,
		tree_node_allocator>::children_iterator::operator++(int) {
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::children_iterator& tree<T,
		tree_node_allocator>::children_iterator::operator+=(unsigned int num) {
	while (num > 0) {
		++(*this);
		--num;
	}
